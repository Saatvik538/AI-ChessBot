...
class Board:
            ...
            for possible_move_col in possible_move_cols:
                if Square.in_range(possible_move_row, possible_move_col):
                    if self.squares[possible_move_row][possible_move_col].has_rival_piece(piece.color):
                        # create inital and ifnal move squares
                        inital = Square(row, col)
            ...
            if not piece.moved:
                ...
                if isinstance(left_rook, Rook):
                    if not left_rook.moved:
                        for c in range(1,4):
                            if self.squares[row][c].has_piece(): #castling isn't possible due to pieces in between
                                break
                            ...
                            if c == 3:
                                ...
                                inital = Square(row, 0)
                                final = Square(row, 3)
                                moveR = Move(inital, final)
                                #king move
                                inital = Square(row, col)
                                final = Square(row, 2)
                                moveK = Move(inital, final)
    ...
    def _create(self): #"_c... shows private method"
        for row in range(ROWS):
            for col in range(COLS):
